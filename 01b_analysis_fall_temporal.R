#title: Fall migration temporal migratory connectivity analysis from Knight et al. 2021. Comprehensive estimation of spatial and temporal migratory connectivity across the annual cycle to direct conservation efforts. Ecography ECOG-05111
...
##2b. Read in list of individuals from fall spatial analysis----
tbl_IDs <- read.csv("FallMigration_Individuals.csv")
...
#save out for later
write.csv(tbl_locs, "CRAWL_MigConnectivity_Fall_Temporal_Points.csv", row.names = FALSE)

#save out as spatial object
sf_locs_wgs <- sf::st_as_sf(tbl_locs, coords = c("long","lat")) %>% 
  sf::st_set_crs(4326)
st_write(sf_locs_wgs, "CRAWL_MigConnectivity_Fall_Temporal_Points.shp", delete_dsn=TRUE)
...
write.csv(PredDataAll, "CRAWL_Fall_Temporal_Predictions.csv", row.names = FALSE)
...
st_write(sf_lines_mean, "CRAWL_Fall_Temporal_Mean.shp", delete_dsn=TRUE)
...
st_write(sf_lines_up, "CRAWL_Fall_Temporal_Upper.shp", delete_dsn=TRUE)
...
st_write(sf_lines_lw, "CRAWL_Fall_Temporal_Lower.shp", delete_dsn=TRUE)
...
ggsave("CRAWL_Fall_Temporal.jpeg", device="jpeg", width=20, height=20, units="in")
...
#inspect data
table(PredData_Fall_Temp$latr, PredData_Fall_Temp$population)
...
write.csv(fall.doys, "Fall_Temporal_Days.csv", row.names = FALSE)
...
write.csv(fall.temporal, "MC_Fall_Temporal_LeaveOneOut.csv", row.names = FALSE)
