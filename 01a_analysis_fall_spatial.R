...
#Visualize sampling for each individual & population
ggplot(tbl_locs_all) +
  geom_point(aes(x=doy_order, y=factor(pinpointid), colour=season, size=duration)) +
  ylab("Pinpoint ID") +
...
tbl_locs_fall <- tbl_locs_all %>% 
  dplyr::filter(season %in% c("FallMig"))

ggplot(tbl_locs_fall, aes(x=duration, y=travdist)) +
  geom_point() +
  geom_smooth() 
#Relationship linear below huge values. Threshold won't work

###2bii. Use rules----
#1) a known wintering site, 2) at least one point in each of North, Central, and South America, and 3) at least one point on either side of the Mississippi region where birds congregate during fall migration before turning south (#3 done by hand)
...
tbl_IDs <- tbl_locs_all %>% 
  ...
  dplyr::filter(winter==1,
                cont==3,
                !pinpointid %in% c(429, 441, 442, 457, 480, 483, 488, 489))

write.csv(tbl_IDs, "FallMigration_Individuals.csv", row.names = FALSE)
...
##3a. Tidy----
#filter to 1) selected individuals, 2) fall migration plus breeding and wintering sites, 3) points less than 1 day apart
tbl_locs <- tbl_locs_all %>% 
  ...
  dplyr::filter(pinpointid %in% tbl_IDs$pinpointid,
                season=="FallMig" | (dplyr::lag(season)=="FallMig" & order != 1) | dplyr::lead(season)=="FallMig",
                duration >= 1) 
...
#save out for later
write.csv(tbl_locs, "CRAWL_Fall_Spatial_Points.csv", row.names = FALSE)

#save out as spatial object
sf_locs_wgs <- sf::st_as_sf(tbl_locs, coords = c("long","lat")) %>% 
  sf::st_set_crs(4326)
st_write(sf_locs_wgs, "CRAWL_Fall_Spatial_Points.shp", delete_dsn=TRUE)
...
write.csv(PredDataAll, "CRAWL_Fall_Spatial_Predictions.csv", row.names = FALSE)
...
st_write(sf_lines_mean, "CRAWL_Fall_Spatial_Mean.shp", delete_dsn=TRUE)
...
st_write(sf_lines_up, "CRAWL_Fall_Spatial_Upper.shp", delete_dsn=TRUE)
...
st_write(sf_lines_lw, "CRAWL_Fall_Spatial_Lower.shp", delete_dsn=TRUE)
...
ggsave("CRAWL_Fall_Spatial.jpeg", device="jpeg", width=20, height=20, units="in")
...
write.csv(fall.lats, "Fall_Spatial_Latitudes.csv", row.names = FALSE)
...
for(h in 1:length(pops)){
    ...
    for(i in 1:nrow(fall.lats.h)){
      
      #latitude for this iteration
      lat.i <- fall.lats.h$latr[i]
    ...
    ##8d. Calculate connectivity----
        ...
        #Report status
        print(paste0("Completed ", i, " of ", nrow(fall.lats), " lats"))
...
write.csv(fall.spatial, "MC_Fall_Spatial_LeaveOneOut.csv", row.names = FALSE)